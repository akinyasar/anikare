...
function getSteps(locale: Locale) {
  return [
    { num: '1', title: t(locale, 'step1Title'), desc: t(locale, 'step1Desc') },
    { num: '2', title: t(locale, 'step2Title'), desc: t(locale, 'step2Desc') },
    { num: '3', title: t(locale, 'step3Title'), desc: t(locale, 'step3Desc') },
  ]
}

function Step({ num, icon, title, desc, index }: { num: string; icon: React.ReactNode; title: string; desc: string; index: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-[#7a6a5a] text-sm leading-relaxed max-w-[220px]">{desc}</p>
    </motion.div>
  )
}
