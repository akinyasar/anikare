...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative overflow-hidden bg-[#FAF7F2] pt-20 pb-12 px-6 md:pt-32 md:pb-20 md:px-10"
    >
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 md:gap-14 items-center">
        ...
        <div className="flex flex-col items-start gap-5 md:gap-7">
          ...
          {/* Heading — soldan, gecikmeli */}
          <motion.h1
            initial={{ opacity: 0, x: -18 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.55, ease: EASE, delay: 0.1 }}
            className="font-[family-name:var(--font-playfair)] text-[2.75rem] sm:text-[3.25rem] font-bold text-[#1a1a1a] leading-[1.1] tracking-tight"
          >
          ...
          <motion.p
            ...
            className="text-lg text-[#7a6a5a] leading-relaxed max-w-lg"
          >
            {t(locale, 'heroDesc')}
          </motion.p>
          ...
          >
            <Link
              href="/giris"
              className="inline-flex justify-center items-center bg-[#6D1A3E] text-white font-semibold px-6 py-3.5 md:px-8 md:py-4 rounded-full hover:bg-[#5a1533] active:scale-[0.97] transition-all shadow-lg shadow-[#6D1A3E]/20"
            >
              {isLoggedIn ? t(locale, 'heroCtaLoggedIn') : t(locale, 'heroCta')}
            </Link>
            <a
              href="#nasil-calisir"
              className="inline-flex justify-center items-center gap-2 text-[#6D1A3E] border border-[#6D1A3E]/25 font-medium px-6 py-3.5 md:px-8 md:py-4 rounded-full hover:bg-[#f5e6ed] hover:border-[#6D1A3E]/50 transition-all"
            >
          ...
          {/* Social proof — aşağıdan */}
          <motion.div
            initial={{ opacity: 0, y: 14 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, ease: EASE, delay: 0.42 }}
            className="flex items-center gap-3 pt-3 border-t border-[#e8ddd5] w-full md:gap-4 md:pt-4"
          >
        ...
        >
          {/* Floating stat card — top left, desktop only */}
          <motion.div
            animate={{ y: [0, -8, 0] }}
          ...
          {/* Floating upload success card — bottom right, desktop only */}
          <motion.div
            animate={{ y: [0, 8, 0] }}
