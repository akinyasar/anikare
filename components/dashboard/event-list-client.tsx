...
export default function EventListClient({ events }: { events: Event[] | null }) {
  const { locale } = useLocale()

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-[#1a1a1a]">{t(locale, 'myEventsTitle')}</h1>
        <Link
          href="/etkinlik/yeni"
          className="bg-[#6D1A3E] text-white px-5 py-2.5 rounded-full font-medium text-sm hover:bg-[#5a1533] transition-colors"
        >
          {t(locale, 'newEventBtn')}
        </Link>
      </div>
      ...
      {!events?.length ? (
        <div className="text-center py-24">
          ...
          <p className="text-[#7a6a5a] text-lg">{t(locale, 'noEvents')}</p>
          <Link
            href="/etkinlik/yeni"
            className="text-[#6D1A3E] font-medium mt-2 inline-block hover:underline"
          >
            {t(locale, 'createFirst')}
          </Link>
        </div>
      ) : (
          ...
          {events.map((event) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                >
                  {event.is_upload_active ? t(locale, 'active') : t(locale, 'closedStatus')}
                </span>
              </div>
